...
def transcribe_audio(audio_file):
    ...
    return {
        ...
        "utterances": [
            {"speaker": str(u.speaker), "text": u.text}
...
def update_speaker_names():
    st.session_state.speaker_names = {
        '1': st.session_state.speaker_1_name,
        '2': st.session_state.speaker_2_name
    }

def reset_session():
    for key in ['transcript_data', 'speaker_names', 'speaker_1_name', 'speaker_2_name', 'show_full_transcript']:
...
def main():
    ...
    if 'speaker_names' not in st.session_state:
        st.session_state.speaker_names = {'1': '', '2': ''}
    ...
    if st.session_state.transcript_data is not None:
        ...
        with col1:
            speaker_1 = st.text_input("Enter name for Speaker 1:", key="speaker_1_name", value=st.session_state.speaker_names['1'])
        with col2:
            speaker_2 = st.text_input("Enter name for Speaker 2:", key="speaker_2_name", value=st.session_state.speaker_names['2'])

        # Buttons in separate rows
        apply_names_disabled = not (speaker_1 and speaker_2)
        ...
        if st.session_state.get('show_full_transcript', False) or (st.session_state.speaker_names['1'] and st.session_state.speaker_names['2']):
            # Display full transcript
            st.subheader("Full transcript:")
            full_transcript = ""
            for utterance in st.session_state.transcript_data["utterances"]:
                if st.session_state.speaker_names['1'] and st.session_state.speaker_names['2']:
